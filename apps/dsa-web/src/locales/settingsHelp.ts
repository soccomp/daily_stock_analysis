...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.ai_model.CODEX_CLI_MODEL': {
    title: 'Codex CLI 模型',
    showFieldKey: true,
    summary: '指定 DSA 通过 Codex CLI 调用的精确模型；生产迁移默认使用 GPT-5.6 Luna。',
    usage: '仅在“分析生成方式”选择 Codex CLI 时生效。保持 gpt-5.6-luna 以使用 ChatGPT OAuth 管理的 Luna 路径。',
    valueNotes: [
      'DSA 不读取 API Key 或 OAuth token；认证由本机 Codex CLI 管理。',
      '配置值会作为单个 argv 参数传给 Codex，不能包含空白或 shell 元字符。',
    ],
    impact: ['影响个股分析、大盘复盘、筛选排序和普通文本生成，不改变 DSA→Athena 的仿真安全边界。'],
    examples: ['CODEX_CLI_MODEL=gpt-5.6-luna'],
  },
  'settings.ai_model.CODEX_CLI_REASONING_EFFORT': {
    title: 'Codex 推理强度',
    showFieldKey: true,
    summary: '指定 Codex CLI 的 reasoning effort；生产默认值为 max。',
  ...
  'settings.ai_model.CODEX_CLI_WEB_SEARCH_ENABLED': {
    title: 'Codex 原生联网搜索',
    showFieldKey: true,
    summary: '是否显式启用 Codex CLI 自带的实时网页搜索。',
    usage: '生产默认关闭；DSA 仍通过自身数据和新闻工具提供证据。',
    valueNotes: ['打开后请求内容可能被发送给网页搜索服务；除非完成额外审计，不建议打开。'],
    impact: ['影响 Codex CLI 是否收到 --search；不改变 DSA 已有数据源的调用。'],
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.ai_model.CODEX_CLI_MODEL': {
    ...
    showFieldKey: true,
    summary: 'Selects the exact model passed to Codex CLI; the production migration default is GPT-5.6 Luna.',
    usage: 'Only applies when Analysis Generation Method is Codex CLI. Keep gpt-5.6-luna for the ChatGPT OAuth-managed Luna path.',
    valueNotes: [
      'DSA does not read an API key or OAuth token; authentication is managed by the local Codex CLI.',
      'The value is passed as one argv token and must not contain whitespace or shell metacharacters.',
    ],
    impact: ['Affects stock analysis, market review, screening ranking, and regular text generation without changing the DSA-to-Athena simulation safety boundary.'],
    examples: ['CODEX_CLI_MODEL=gpt-5.6-luna'],
  },
  'settings.ai_model.CODEX_CLI_REASONING_EFFORT': {
    title: 'Codex Reasoning Effort',
    showFieldKey: true,
    summary: 'Selects Codex CLI reasoning effort; the production default is max.',
  ...
  'settings.ai_model.CODEX_CLI_WEB_SEARCH_ENABLED': {
    ...
    showFieldKey: true,
    summary: 'Controls whether Codex CLI native live web search is explicitly enabled.',
    usage: 'Disabled by default in production; DSA continues to provide evidence through its own data and news tools.',
    valueNotes: ['When enabled, request content may be sent to web search services; do not enable without a separate audit.'],
    impact: ['Controls whether Codex CLI receives --search and does not change DSA-owned data-source calls.'],
    examples: ['CODEX_CLI_WEB_SEARCH_ENABLED=false'],
  },
  'settings.ai_model.GENERATION_BACKEND_TIMEOUT_SECONDS': {
