...
- Pallas production uses `GENERATION_BACKEND=codex_cli`, an empty `GENERATION_FALLBACK_BACKEND`, `CODEX_CLI_MODEL=gpt-5.6-luna`, and `CODEX_CLI_REASONING_EFFORT=max`. Codex manages ChatGPT OAuth; DSA never reads, refreshes, logs, or forwards OAuth tokens to LiteLLM/OpenAI API. Authentication and quota failures remain explicit (`login_required` / `quota_exhausted`) and never switch silently to another provider or local model.
