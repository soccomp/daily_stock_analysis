...
- Pallas convergence uses `GENERATION_BACKEND=codex_cli`, an empty `GENERATION_FALLBACK_BACKEND`, `CODEX_CLI_MODEL=gpt-5.6-luna`, and `CODEX_CLI_REASONING_EFFORT=xhigh`. Codex manages ChatGPT OAuth; DSA never reads, refreshes, logs, or forwards OAuth tokens to LiteLLM/OpenAI API. Authentication and quota failures remain explicit (`login_required` / `quota_exhausted`) and never switch silently to another provider or local model.
...
- The current `codex_cli` preset uses `codex exec --skip-git-repo-check --sandbox read-only --color never --ephemeral --json --output-last-message <temp-file> -`; model and `model_reasoning_effort="xhigh"` are injected by the controlled runtime, and the removed `--ask-for-approval` flag is no longer used. DSA reads the final response from the temporary file and never uses duplicate stdout for main-analysis JSON parsing. The locally verified version is `codex-cli 0.149.0-alpha.4.1`; incompatible CLI contracts return structured errors and the Pallas profile does not switch providers automatically.
