...
def test_qwen_identity_metadata_cannot_promote_failed_generation(tmp_path):
    store = DependencyHealthStore(tmp_path / "qwen-health.json", transition_cooldown_seconds=0)
    store.record_result(
        "qwen-omlx",
    ...
    row = store.snapshot()["dependencies"]["qwen-omlx"]
...
def test_qwen_generation_recovery_requires_a_new_generation_observation(tmp_path):
    store = DependencyHealthStore(tmp_path / "qwen-health.json", transition_cooldown_seconds=0)
    store.record_result(
        "qwen-omlx", category="LLM_RESEARCH", success=False, reachable=True, usable=False,
        failure_class_name="TIMEOUT", observation_kind="generation",
    )
    store.record_result(
        "qwen-omlx", category="LLM_RESEARCH", success=True, reachable=True, usable=True,
...
def test_qwen_generation_health_expires_without_new_generation(tmp_path):
    store = DependencyHealthStore(tmp_path / "qwen-expiry.json", transition_cooldown_seconds=0)
    store.record_result(
        "qwen-omlx", category="LLM_RESEARCH", success=True, reachable=True, usable=True,
        records=1, observation_kind="identity",
    )
    ...
    assert store.snapshot()["dependencies"]["qwen-omlx"]["status"] == "STALE"


def test_metadata_monitor_observation_does_not_clear_generation_failure(monkeypatch, tmp_path):
    from src.services.dependency_health import DependencyHealthMonitor

    store = DependencyHealthStore(tmp_path / "monitor-qwen.json", transition_cooldown_seconds=0)
    monkeypatch.setenv("LLM_OMLX_BASE_URL", "http://127.0.0.1:8000/v1")
    store.record_result(
        "qwen-omlx", category="LLM_RESEARCH", success=False, reachable=True, usable=False,
        failure_class_name="TIMEOUT", observation_kind="generation",
    )
    monkeypatch.setattr(
        "src.services.qwen_health.probe_qwen_identity",
        lambda: {
            "configured": True, "enabled": True, "endpoint": "http://127.0.0.1:8000/v1",
            "success": True, "reachable": True, "usable": True, "loaded_count": 1,
        },
    )
    snapshot = DependencyHealthMonitor(store).once()
    assert snapshot["dependencies"]["qwen-omlx"]["identity_status"] == "HEALTHY"
    assert snapshot["dependencies"]["qwen-omlx"]["generation_status"] == "FAILED"
    assert snapshot["dependencies"]["qwen-omlx"]["status"] == "FAILED"
